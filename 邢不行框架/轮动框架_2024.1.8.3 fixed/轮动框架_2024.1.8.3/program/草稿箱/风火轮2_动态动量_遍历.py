...
sub_stg_list = ['笛卡尔2', '小市值', '小市值_基本面优化']  # 子策略名称，如果为空或者没有这个字段，表示使用所有的子策略
...
def cal_strategy_factors(data, exg_dict):
    ...
    para_list = []
    for s in range(2, 31, 2):
        for l in range(120, 251, 10):
            para_list.append([s, l])
    ...
    for para in para_list:
        ...
        data[f'mtm_std_{short}_rank{long}'] = data[f'mtm_std_{short}'].rolling(window=long, min_periods=long).rank(
            pct=True)
        ...
        data[f'ret_std_{short}_rank{long}'] = data[f'ret_std_{short}'].rolling(window=long, min_periods=long).rank(
            pct=True)
