...
# 计算财务预处理数据
def merge_with_finance_data(stock_df, code, finance_data_path, add_columns, extra_agg_dict, flow_fin_cols=[],
                            cross_fin_cols=[]):
    """
    将财务数据合并到日线数据上
    :param stock_df: 日线数据
    ...
    :param add_columns: 需要添加到日线数据的列
    :param extra_agg_dict: 合并周（月）线所需要的参数
    :param flow_fin_cols: 流量型财务数据
    :param cross_fin_cols: 截面型财务数据
    :return:
    """
    # 判断路径是否存在
    ...
    if os.path.exists(file_path):
        ...
        for file in fin_files:
            finance_df = pd.read_csv(f'{file_path}/{file}', parse_dates=['publish_date'], skiprows=1,
                                     encoding='gbk')  # 读取财务数据
            # 判断财务数据中是否包含我们需要的finance_cols
            for col in set(flow_fin_cols + cross_fin_cols + add_columns):
                if col not in finance_df.columns:
                    finance_df[col] = np.nan

            necessary_cols = ['stock_code', 'report_date', 'publish_date']
            finance_df = finance_df[list(set(necessary_cols + flow_fin_cols + cross_fin_cols + add_columns))]
            finance_df = cal_fin_data(data=finance_df, flow_fin_list=flow_fin_cols, cross_fin_list=cross_fin_cols,
                                      discard=False)
            # 合并
            col = ['publish_date', 'report_date'] + add_columns
        ...
        all_finance_df['废弃报告'] = mark_old_report(all_finance_df['report_date'])
        # 删除废弃的研报
        ...
        for col in ['publish_date', 'report_date'] + add_columns:
