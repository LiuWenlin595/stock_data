...
factors = {'ROE': ['ROE_单季'], '归母净利润同比增速': ['归母净利润同比增速_60'], '成交额相关因子': ['成交额_Mean5']}  # 策略要用到的因子
...
def select_stock(all_data, count, params=[]):
    ...
    # 计算ROE排名
    all_data['ROE_单季排名'] = all_data.groupby('交易日期')['ROE_单季'].rank(ascending=False, method='min', pct=True)
    # 去除ROE较差的20%的股票
    all_data = all_data[all_data['ROE_单季排名'] < 0.8]
    # 计算总市值排名
    all_data['总市值排名'] = all_data.groupby('交易日期')['总市值'].rank(ascending=True, method='min')
    # 计算归母净利润同比增速排名
    all_data['归母净利润同比增速排名'] = all_data.groupby('交易日期')['归母净利润同比增速_60'].rank(ascending=False, method='min')
    # 计算复合因子
    all_data['复合因子'] = all_data['总市值排名'] + all_data['归母净利润同比增速排名']
