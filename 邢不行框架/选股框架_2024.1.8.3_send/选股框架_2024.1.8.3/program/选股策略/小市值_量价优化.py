...
factors = {'alpha95': ['alpha95'], 'Ret': ['Ret_20'], '成交额相关因子': ['成交额_Mean5']}  # 策略要用到的因子
...
def select_stock(all_data, count, params=[]):
    ...
    # 计算alpha95排名  衡量低波动
    all_data['alpha95排名'] = all_data.groupby('交易日期')['alpha95'].rank(ascending=True, method='min')
    # 计算Ret20排名  衡量超跌
    all_data['Ret_20排名'] = all_data.groupby('交易日期')['Ret_20'].rank(ascending=True, method='min')
    # 计算复合因子
    all_data['复合因子'] = all_data['alpha95排名'] + all_data['总市值排名'] + all_data['Ret_20排名']
