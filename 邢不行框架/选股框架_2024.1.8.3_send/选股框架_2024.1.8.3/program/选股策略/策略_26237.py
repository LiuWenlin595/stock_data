...
# 来源: https://qtcls.cn/26237

# 持仓周期以及对应的offset，必须要在period_offset.csv中有对应的列，例如：W_0,W_1,W_2,W_3,W_4
period_offset = ['W_0']

factors = {'Ret': ['Ret_20'], '换手率': ['周期换手率'], 'ROE': ['ROE_单季'], '归母净利润': ['归母净利润同比增速_60'], '现金流负债比': ['现金流负债比'],
           '成交额相关因子': ['成交额_Mean5']}  # 策略要用到的因子
...
def select_stock(all_data, count, params=[]):
    ...
    # 筛选出市值小于25%分位数
    all_data['市值25分位数'] = all_data.groupby('交易日期')['总市值'].transform(lambda x: x.quantile(0.25))
    con1 = (all_data['总市值'] < all_data['市值25分位数'])
    all_data = all_data[con1]
    ...
    con2 = (all_data['ROE排名'] < 0.8)
    all_data = all_data[con2]

    # 计算现金流负债比：负债偿还能力，资金利用能力，合理区间
    con3 = (all_data['现金流负债比'] < 0.8)
    all_data = all_data[con3]

    # 周期换手率
    con4 = (all_data['周期换手率'] < 0.6)
    all_data = all_data[con4]

    # 计算总市值排名
    all_data['总市值排名'] = all_data.groupby('交易日期')['总市值'].rank(ascending=True, method='min')

    # 计算归母净利润同比增速排名
    all_data['归母净利润同比增速排名'] = all_data.groupby('交易日期')['归母净利润同比增速_60'].rank(ascending=False,
                                                                            method='min')

    # 计算Ret20排名  衡量超跌
    all_data['Ret_20排名'] = all_data.groupby('交易日期')['Ret_20'].rank(ascending=True, method='min')
    # 计算复合因子
    all_data['复合因子'] = all_data['总市值排名'] + all_data['归母净利润同比增速排名'] + all_data['Ret_20排名']

    # 对因子进行排名
    all_data['选股排名'] = all_data.groupby('交易日期')['复合因子'].rank(method='first')
