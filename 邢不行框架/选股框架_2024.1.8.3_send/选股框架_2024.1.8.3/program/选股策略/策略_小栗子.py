...
factors = {'归母净利润': ['归母净利润同比增速_60'], 'ROE': ['ROE_单季'], 'Ret': ['Ret_21'], '资金流因子': ['中户买入占比'],
           '换手率': ['换手率5日均值'], 'WR': ['WR_5'], 'JS': ['JS_5'], '成交额相关因子': ['成交额_Std20', '成交额_Mean5']}  # 策略要用到的因子
...
def select_stock(all_data, count, params=[]):
    ...
    all_data = all_data[all_data['总市值'] < 3e9]
    all_data = all_data[all_data['换手率5日均值'] > 0.01]

    # 筛选出归母净利润同比增速_60后80%的股票
    all_data['归母净利润同比增速_60排名'] = all_data.groupby('交易日期')['归母净利润同比增速_60'].rank(ascending=False, method='min', pct=True)
    condition = (all_data['归母净利润同比增速_60排名'] < 0.8)
    # 筛选出收盘价后95%的股票
    all_data['收盘价_排名'] = all_data.groupby('交易日期')['收盘价'].rank(ascending=False, method='min', pct=True)
    condition &= (all_data['收盘价_排名'] < 0.95)
    # 筛选出ROE_单季后95%的股票
    all_data['ROE排名'] = all_data.groupby('交易日期')['ROE_单季'].rank(ascending=False, method='min', pct=True)
    condition &= (all_data['ROE排名'] < 0.95)
    # 筛选出WR的前80%的股票
    all_data["WR_5_排名"] = all_data.groupby("交易日期")["WR_5"].rank(ascending=True, method="min", pct=True)
    condition &= (all_data["WR_5_排名"] > 0.2)
    # 筛选出JS的后80%的股票
    all_data["JS_5_排名"] = all_data.groupby("交易日期")["JS_5"].rank(ascending=True, method="min", pct=True)
    condition &= (all_data["JS_5_排名"] < 0.8)
    # 计算总市值排名
    all_data['总市值排名'] = all_data.groupby('交易日期')['总市值'].rank(ascending=True, method='min')
    # 计算Ret21排名
    all_data['Ret_21排名'] = all_data.groupby('交易日期')['Ret_21'].rank(ascending=True, method='min')
    # 计算成交额排名
    all_data['成交额std_20_排名'] = all_data.groupby('交易日期')['成交额_Std20'].rank(ascending=True, method='min')
    # 中户资金排名
    all_data['中户买入占比排名'] = all_data.groupby('交易日期')['中户买入占比'].rank(ascending=False, method='min')

    all_data = all_data[condition]
    all_data['复合因子'] = all_data['总市值排名'] + all_data['中户买入占比排名'] + all_data['成交额std_20_排名'] + all_data['Ret_21排名']
