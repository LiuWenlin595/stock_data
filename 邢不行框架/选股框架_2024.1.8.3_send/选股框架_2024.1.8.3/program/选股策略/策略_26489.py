...
# 来源: https://qtcls.cn/26489

# 持仓周期以及对应的offset，必须要在period_offset.csv中有对应的列，例如：W_0,W_1,W_2,W_3,W_4
period_offset = ['W_0']

factors = {'Ret': ['Ret_20'], '成交额相关因子': ['成交额_Std5', '周期成交额', '成交额_Mean5']}  # 策略要用到的因子
...
def select_stock(all_data, count, params=[]):
    ...
    # 计算成交额_Std5排名  衡量成交量低波动
    all_data['成交额std_5排名'] = all_data.groupby('交易日期')['成交额_Std5'].rank(ascending=True, method='min')
    # 计算市成交量排名
    all_data['成交额排名'] = all_data.groupby('交易日期')['周期成交额'].rank(ascending=True, method='min')
    # 计算复合因子
    all_data['复合因子'] = all_data['总市值排名'] + all_data['Ret_20排名'] + all_data['成交额std_5排名'] + all_data['成交额排名']
