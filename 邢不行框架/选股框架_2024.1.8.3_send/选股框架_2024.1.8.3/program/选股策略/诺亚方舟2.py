...
def select_stock(all_data, count, params=[]):
    ...
    # 过滤条件
    all_data['总市值排名'] = all_data.groupby('交易日期')['总市值'].rank(ascending=False, pct=True)
    all_data = all_data[all_data['总市值排名'] <= 0.1]

    # 选股条件
    all_data['Tur_20排名'] = all_data.groupby('交易日期')['Tur_20'].rank(ascending=True, pct=True)
    all_data['胜率_120排名'] = all_data.groupby('交易日期')['胜率_120'].rank(ascending=False, pct=True)
    # all_data['均线胜率_60排名'] = all_data.groupby('交易日期')['均线胜率_60'].rank(ascending=False, pct=True)

    con1 = all_data['Tur_20排名'] <= 0.1
    con2 = all_data['胜率_120排名'] <= 0.1
    all_data = all_data[con1 & con2]

    # === 计算复合因子
    all_data['复合因子'] = all_data['Tur_20排名'] + all_data['胜率_120排名']
    # 删除因子为空的数据
