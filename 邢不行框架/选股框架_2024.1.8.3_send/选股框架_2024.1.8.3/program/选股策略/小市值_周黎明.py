...
factors = {'Ret': [], 'alpha95': [], '成交额相关因子': ['成交额_Mean5']}
...
def select_stock(all_data, count, params=[]):
    ...
    """
    # --------------------------------------------------------------------
    # 原策略请查看帖子：https://bbs.quantclass.cn/thread/25799
    # --------------------------------------------------------------------
    all_data = all_data[all_data['成交额_Mean5'] > 50000000]

    all_data['Ret5排名'] = all_data.groupby('交易日期')['Ret_5'].rank(ascending=False)
    all_data['排名靠前'] = np.where(all_data['Ret5排名'] <= 100, 1, 0)
    ind_strength = all_data.groupby(['交易日期', '新版申万一级行业名称'])['排名靠前'].sum().reset_index()
    ind_strength['排名靠前_排名'] = ind_strength.groupby('交易日期')['排名靠前'].rank(ascending=False)
    ind_strength = ind_strength[ind_strength['排名靠前_排名'] <= 2]
    all_data = pd.merge(all_data, ind_strength[['交易日期', '新版申万一级行业名称']], on=['交易日期', '新版申万一级行业名称'])

    # 计算Ret20排名  衡量超跌
    all_data['Ret_20分位数'] = all_data.groupby('交易日期')['Ret_20'].rank(ascending=False, method='min', pct=True)
    all_data = all_data[all_data['Ret_20分位数'] > 0.2]

    # 小市值分域，选择市值排名前10%
    all_data['总市值排名'] = all_data.groupby('交易日期')['总市值'].rank(ascending=True, method='min')

    # 计算复合因子
    all_data['复合因子'] = all_data['总市值排名']
