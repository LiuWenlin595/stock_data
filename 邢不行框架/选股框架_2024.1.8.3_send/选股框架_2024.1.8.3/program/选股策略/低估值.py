...
def select_stock(all_data, count, params=[]):
    ...
    """
    # 市盈率做排名
    all_data['EP排名'] = all_data.groupby(['交易日期'])['EP'].rank(ascending=False)
    # 市净率做排名
    all_data['BP排名'] = all_data.groupby(['交易日期'])['BP'].rank(ascending=False)

    all_data['复合因子'] = all_data['EP排名'] + all_data['BP排名']

    # 对因子进行排名
    all_data['选股排名'] = all_data.groupby('交易日期')['复合因子'].rank(method='first', ascending=True)
