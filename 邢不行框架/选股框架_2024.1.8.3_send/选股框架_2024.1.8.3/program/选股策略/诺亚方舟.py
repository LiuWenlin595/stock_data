...
def select_stock(all_data, count, params=[]):
    ...
    # 选股条件
    all_data['均线胜率排名'] = all_data.groupby('交易日期')['均线胜率_60'].rank(ascending=False)
    all_data['异常涨跌幅排名'] = all_data.groupby('交易日期')['异常涨跌幅_250'].rank(ascending=True)

    # === 计算复合因子
    all_data['复合因子'] = all_data['异常涨跌幅排名'] + all_data['均线胜率排名']
    # 删除因子为空的数据
