...
def select_stock(all_data, count, params=[]):
    ...
    all_data = all_data[all_data['股票代码'].str.contains('sh68') == False]
    all_data = all_data[all_data['股票代码'].str.contains('sz30') == False]
    # ===构建策略需要的因子
    all_data['总市值_排名'] = all_data.groupby('交易日期')['总市值'].rank(ascending=True, method='min')

    all_data = all_data[all_data['总市值_排名'] <= 100]
    all_data['上市至今交易天数_排名'] = all_data.groupby('交易日期')['上市至今交易天数'].rank(ascending=True, method='first')

    # === 计算复合因子
    all_data['复合因子'] = all_data['上市至今交易天数_排名']
