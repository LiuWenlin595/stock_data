...
# 计算财务预处理数据
def merge_with_finance_data(stock_df, code, finance_data_path, ipt_fin_cols, opt_fin_cols, extra_agg_dict={}):
    """
    将财务数据合并到日线数据上
    :param stock_df: 日线数据
    :param code: 股票代码
    :param finance_data_path:财务数据路径
    :param ipt_fin_cols: 输入的财务字段
    :param opt_fin_cols: 输出的财务字段
    :param extra_agg_dict: 合并周（月）线所需要的参数
    :return:
    """
    # 判断路径是否存在
    file_path = os.path.join(finance_data_path, code)
    if os.path.exists(file_path):
        # 获取路径下的所有文件名
        fin_files = os.listdir(file_path)
        finance_dfs = []
        # 划分流量型和截面型财务数据
        flow_fin_cols = [col for col in ipt_fin_cols if (col.startswith('R_')) or (col.startswith('C_'))]   # 流量型
        cross_fin_cols = [col for col in ipt_fin_cols if col.startswith('B_')]                              # 截面型
        # 读取路径下的各个财务数据文件
        for file in fin_files:
            finance_df = pd.read_csv(os.path.join(file_path, file), parse_dates=['publish_date'], skiprows=1,
                                     encoding='gbk')  # 读取财务数据
            # 判断财务数据中是否包含我们需要的finance_cols
            for col in set(flow_fin_cols + cross_fin_cols + opt_fin_cols):
            ...
            finance_df = finance_df[list(set(necessary_cols + flow_fin_cols + cross_fin_cols + opt_fin_cols))]   # 取需要的数据
            # 计算财务类因子
            finance_df = cal_fin_data(data=finance_df, flow_fin_list=flow_fin_cols, cross_fin_list=cross_fin_cols,
                                      discard=False)
            # 合并
            col = ['publish_date', 'report_date'] + opt_fin_cols
        ...
        print(f'{code}未找到财务数据，如果一直报这个错误，请检查财务数据路径是否正确。偶尔几个可以忽略。')
        all_finance_df = pd.DataFrame()
        for col in ['publish_date', 'report_date'] + opt_fin_cols:
            stock_df[f'{col}'] = np.nan
            all_finance_df[f'{col}'] = np.nan
        all_finance_df_not_discord = all_finance_df.copy()

    # ===整理extra_agg_dict
    for col in opt_fin_cols:
